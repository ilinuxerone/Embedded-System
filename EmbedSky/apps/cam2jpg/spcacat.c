...
struct frame_t *headerframe;
  for (;;)
	...
	if(overwrite) {
	getJpegPicture(pictureData,headerframe->w,headerframe->h,VIDEO_PALETTE_JPEG,headerframe->size,PICTWRD,NULL,0);
	} else {
	getJpegPicture(pictureData,headerframe->w,headerframe->h,VIDEO_PALETTE_JPEG,headerframe->size,PICTURE,NULL,0);
	}
	pictFlag = 0;
	videoIn.framelock[iframe]--;
	}
